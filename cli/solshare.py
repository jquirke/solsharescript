...
def print_table(data):
    ...
    print()
    print('┌─────────────────────┬────────┬────────┬────────┬────────┬────────┬──────────────────────────┐')
    print(f'│ {time_hdr:<19} │ Demand │ Delivrd│ Exportd│  Solar │   Grid │ Solar coverage           │')
    print('├─────────────────────┼────────┼────────┼────────┼────────┼────────┼──────────────────────────┤')
    ...
    for r in data:
        ...
        print(f'│ {t.strftime("%a %d %b %H:%M"):19} │ {demand:5.2f}  │ {delivered:5.2f}  │ {exported:5.2f}  │ {solar:5.2f}  │ {grid:5.2f}  │ {b} {pct:3.0f}% │')
        total_demand    += demand
        total_solar     += solar
    ...
    print('├─────────────────────┼────────┼────────┼────────┼────────┼────────┼──────────────────────────┤')
    print(f'│ {"TOTAL":19} │ {total_demand:5.2f}  │ {total_delivered:5.2f}  │ {total_exported:5.2f}  │ {total_solar:5.2f}  │ {total_grid:5.2f}  │ {"Overall solar:":14} {overall_pct:3.0f}%    │')
    print('└─────────────────────┴────────┴────────┴────────┴────────┴────────┴──────────────────────────┘')
    print()
...
def parse_dt(s, is_to=False):
    """Parse YYYY-MM-DDTHH:MM or YYYY-MM-DD into a local datetime.
    For date-only --to values, advances to end of day (midnight next day)."""
    for fmt, date_only in [("%Y-%m-%dT%H:%M", False), ("%Y-%m-%d", True)]:
        try:
            dt = datetime.strptime(s, fmt).replace(tzinfo=LOCAL_TZ)
            if date_only and is_to:
                dt += timedelta(days=1)
            return dt
        except ValueError:
            continue
    print(f"Invalid date '{s}'. Use YYYY-MM-DDTHH:MM or YYYY-MM-DD.", file=sys.stderr)
    sys.exit(1)


def date_range(from_str, to_str):
    """Parse date/datetime strings in local time, return UTC unix seconds."""
    return int(parse_dt(from_str).timestamp()), int(parse_dt(to_str, is_to=True).timestamp())
